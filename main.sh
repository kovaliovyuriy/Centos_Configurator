#!/bin/bash
ver="v1.0"
title="CentOS конфигуратор сервера"
...
about()
{
echo "Данный скрипт предназначен для быстрой настройки серверов CentOS.
"
}
changelog()
{
...
{
wget https://kovaliov.tk/$filename -r -N -nd --no-check-certificate
chmod 777 $filename
sh $0
exit
}
...
│ ├───┼──────────────────────────────────────┤
├─┤ 5 │ Проверка S.M.A.R.T.                  │
│ ├───┼──────────────────────────────────────┤
├─┤ 6 │ Описание теста производительности    │
│ ├───┼──────────────────────────────────────┤
...
├─┤ 2 │ Обновить ОС                          │
│ ├───┼──────────────────────────────────────┤
├─┤ 3 │ Установить нужные приложения (пакеты)│
│ ├───┼──────────────────────────────────────┤
├─┤ 4 │ Антивирус                            │
│ ├───┼──────────────────────────────────────┤
├─┤ 5 │ Firewall (iptables)                  │
...
myread_dig pick
	...
	5) #Проверка S.M.A.R.T.
		...
		if [ $exist == false ]; then
			echo "Сейчас будет произведена установка программы smartmontools. Но для её установки нужно наличие добавленного репозитория EPEL."
			echo "Если вы уже добавляли репозитории сами или с помощью этой программы, то от вас ничего не требуется. В противном случае, их нужно добавить."
			echo "Добавить репозитории?"
			myread_yn pick
			case "$pick" in
		...
		my_clear
		echo "Сейчас будет произведена проверка S.M.A.R.T. . Ждите..."
		br
		smartctl -d ata -a -i /dev/sda
		br
		smartctl -d ata -a -i /dev/sdb
		br
		smartctl -d ata -a -i /dev/sdc
		br
		smartctl -d ata -a -i /dev/sdd
		br
		wait
    ;;
    6) #Описание теста производительности
...
pick=$chosen2
    ...
    wait    
    ;;
    3) #Установить необходимые приложения (пакеты)
	...
	echo "tmux (консольная утилита-мультиплексор)"
	echo "iotop (отображает статистику использования дискового пространства)"
	echo "iftop (контроль трафика в режиме реального времени online)"
	echo "nload (просмотр загрузки канала в консольном режиме)"
	echo "rsync (выполняет эффективно синхронизацию файлов и каталогов в двух местах)"
	echo "smartmontools (Мониторинг состояния жестких дисков)"
